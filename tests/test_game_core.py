...
class StrategyAndPersistenceTests(unittest.TestCase):
    def test_factory_builds_vesper_strategy(self) -> None:
        strategy = StrategyFactory.create("vesper", owner_name="Vesper", rng=random.Random(1))
    ...
    def test_elimination_narration_takes_priority_over_generic_challenge(self) -> None:
        engine = GameEngine(random.Random(19))
        state = engine.bootstrap_match("Player")
        player = state.players[0]
        opponent = state.players[1]

        for extra_player in state.players[2:]:
            extra_player.eliminated = True
            extra_player.receive_hand([])

        state.current_turn_index = player.seat_index
        player.hand = [Card(rank=Rank.ACE, suit=Suit.SPADES)]
        opponent.hand = [Card(rank=Rank.KING, suit=Suit.CLUBS)]
        player.reputation = 50
        opponent.reputation = 50
        opponent.revolver = Revolver(chambers=[ChamberResult.LOADED])
        ...
        engine.process_action(opponent, TurnAction.challenge())

        recent_lines = [event.text for event in state.narration_log[-2:]]
        self.assertTrue(
            any(line in DEALER_EVENT_LINES["elimination"] for line in recent_lines)
        )
...
class EngineSpecialCardTests(unittest.TestCase):
    def setUp(self) -> None:
        ...
        self.player.eliminated = False
        self.opponent.eliminated = False
    ...
    def test_double_down_causes_two_bullet_spins_on_truthful_claim(self) -> None:
        ...
        self.opponent.revolver = Revolver(chambers=[ChamberResult.EMPTY, ChamberResult.EMPTY])
